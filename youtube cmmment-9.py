...
def get_video_title(youtube_url):
    try:
        # Parse the URL and extract only the 'v' parameter
        parsed_url = urllib.parse.urlparse(youtube_url)
        query_params = urllib.parse.parse_qs(parsed_url.query)
        video_id = query_params.get("v", [None])[0]

        if not video_id:
            raise ValueError("Missing 'v' parameter in the YouTube URL.")

        # Rebuild a clean URL with only the video ID
        clean_url = f"https://www.youtube.com/watch?v={video_id}"

        # Use yt-dlp to extract the title
